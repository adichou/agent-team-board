...
const EN = {
  ...
  // BUG-20260914-017：同步范围（排除 main）常驻说明 + 悬停提示补译（title 原为
  ...
  '批量任务': 'Batch tasks',
  'AI 分析': 'AI analysis',
  // REQ-20260914-001 挂起确认（自动提交不完整 / AI 分析问题）：任务页卡片 + 统一侧拉面板
  '待人工确认提交': 'Pending commit confirmation',
  '待人工确认分析': 'Pending analysis confirmation',
  '部分提交，开发未完成': 'Partially committed — development not complete',
  '部分提交，开发未完成——文档或部分文件提交不构成完成': 'Partially committed — development not complete; docs or partial files do not count as done',
  '查看并确认': 'Review & confirm',
  '重新核验': 'Re-verify',
  '正在核验提交与测试…': 'Verifying commits & tests…',
  '保持挂起': 'Keep suspended',
  '确认并继续': 'Confirm & continue',
  '保存草稿': 'Save draft',
  '已确认恢复': 'Confirmed & resumed',
  '已确认续跑': 'Confirmed & continuing',
  '确认中…': 'Confirming…',
  '已入库': 'Committed',
  '未提交': 'Uncommitted',
  '查看差异': 'View diff',
  ...
  '核对后确认：核验通过才恢复队列': 'Review then confirm: the queue resumes only after verification passes',
  '核对文件与差异后确认：服务端将重新核验、补交并验证测试，通过后恢复队列': 'Review files & diffs, then confirm: the server re-verifies, supplements commits and runs tests before resuming the queue',
  '逐项作答后确认：答案回传当前条目继续分析，未决问题清零才处理下一条': 'Answer each question, then confirm: answers return to the current analysis; the next item starts only after all questions are settled',
  '逐项作答后确认：答案回传当前条目继续分析，未决问题清零才处理下一条': 'Answer each question, then confirm: answers return to the current analysis; the next item starts only after all questions are settled',
  '正在读取挂起详情…': 'Loading suspension details…',
  '已保持挂起：现场与队列暂停保留（取消队列须显式终止任务或恢复领取）': 'Kept suspended: the scene and queue pause remain (cancelling the queue requires an explicit action)',
  '处理说明（保持挂起时记录；确认时可留空）': 'Handling note (recorded when kept suspended; optional on confirm)',
  '处理说明（保持挂起时记录）': 'Handling note (recorded when kept suspended)',
  '如：已核对归属，整文件归本单': 'e.g. ownership reviewed; the whole file belongs to this item',
  '请至少填写或选择一项再保存草稿': 'Fill in or select at least one answer before saving the draft',
  '草稿已保存：必答已齐，可「确认并继续」': 'Draft saved: all required answers in place — you can "Confirm & continue"',
  '请完成作答后再确认': 'Complete the answers before confirming',
  '推荐': 'recommended',
  '（推荐）': '(recommended)',
  '（选答）': '(optional)',
  '选择方案，可在此补充意见': 'Pick an option; add remarks here if any',
  '人工答复（必答）': 'Human answer (required)',
  '问题': 'Questions',
  '（与 HEAD 一致：无差异——可能已补交入库）': '(identical to HEAD: no diff — likely already supplemented)',
  // REQ-20260914-001 补充：卡片 / 面板 / 反馈片段（coverage 拆分片段逐一登记）
  '待确认提交': 'pending commit confirmation',
  '待确认分析': 'pending analysis confirmation',
  '分析问题待人工确认，队列已挂起': 'analysis question awaiting confirmation; queue suspended',
  '自动提交不完整，队列已挂起': 'auto-commit incomplete; queue suspended',
  '关闭挂起确认面板': 'Close suspension confirmation panel',
  '可留空': 'optional',
  '无需补交（已完整入库）': 'nothing to supplement (already fully committed)',
  '尚未核验：确认时将自动重新核验并运行测试': 'Not verified yet: confirmation re-verifies and runs tests automatically',
  '正在读取差异…': 'Loading diff…',
  '未通过': 'failed',
  '见卡片与面板明细': 'see the card and panel details',
  '见面板明细': 'see the panel details',
  '· 历史账本恢复': '· recovered from the historical ledger',
  '· 已确认，答案续跑中': '· confirmed; answers resuming',
  ...
  '已复制完整 Git 错误（原始输出保留用于诊断）': 'Full Git error copied (raw output kept for diagnosis)',
  '复制失败：浏览器未授予剪贴板权限（可手动选中展开区复制）': 'Copy failed: no clipboard permission (select the expanded text and copy manually)',
  '该错误在登记前已被截断且无原始日志可回溯：信息不足，不据此推断具体原因（不编造原因）。': 'The error was truncated before registration and no raw log survives: information is insufficient — no cause is inferred (none is fabricated).',
  '（无摘要）': '(no summary)',
...
const EN_DYNAMIC = {
  ...
  '挂起详情读取失败：◇': 'Failed to load suspension details: $1',
  '挂起确认清单加载失败：◇': 'Failed to load suspension confirmations: $1',
  '待人工确认提交 · ◇': 'Pending commit confirmation · $1',
  '待人工确认分析 · ◇': 'Pending analysis confirmation · $1',
  '背景：◇': 'Background: $1',
  '问题 ◇ 项 · 已答 ◇ · 必答未答 ◇◇': '$1 questions · answered $2 · required unanswered $3$4',
  '◇ 项问题（必答未答 ◇）◇': '$1 questions (required unanswered: $2)$3',
  '◇ 项必答问题': '$1 required questions',
  '最近核验（◇）：通过': 'Last verification ($1): passed',
  '最近核验（◇）：未通过': 'Last verification ($1): failed',
  '已提交：◇ 组 · 待人工：◇ 路径◇': 'Committed: $1 groups · pending manual: $2 paths$3',
  ...
  '已提交：◇ 组（补交 ◇ 组）· ◇◇': 'Committed: $1 groups ($2 supplemented) · $3$4',
  '待提交：◇ 个路径': 'to commit: $1 paths',
  '◇（本单可归属 ◇ · 归属待确认 ◇）': '$1 (attributable to this item: $2 · ownership to confirm: $3)',
  '本单可归属（◇）': 'Attributable to this item ($1)',
  '归属待确认（◇）——全局 / 来源不明，不得静默归为本单': 'Ownership to confirm ($1) — global / unknown origin; never silently attributed to this item',
  '待核对：◇ 个归属待确认路径尚未选择计入 / 排除。': 'To determine: $1 ownership-uncertain path(s) still lack an include/exclude choice.',
  '确认范围（提交前展示实际将提交的文件）：本单可归属 ◇ 个 + 归属待确认已计入 ◇ 个 = 将补交 ◇ 个路径': 'Confirm scope (files actually to be committed, shown before committing): $1 attributable to this item + $2 ownership-confirmed included = $3 paths to supplement',
  '（排除 ◇ 个）': ' ($1 excluded)',
  '；◇ 个归属待确认未处理（须逐项选择计入 / 排除才能确认）': '; $1 ownership-uncertain path(s) unresolved (each must be set to include/exclude before confirming)',
  ...
  '◇阻塞': '$1 block',
  '◇（补充：◇）': '$1 (note: $2)',
  '◇：◇；到任务页「待人工确认」完成确认': '$1: $2; complete the confirmation in the "Pending confirmations" area of the tasks page',
  '⚠ 待人工确认（◇）——阻塞队列，确认后才继续': '⚠ Pending confirmations ($1) — queue blocked; resumes only after confirmation',
  '✓ ◇ 已确认恢复：队列继续': '✓ $1 confirmed & resumed: queue continues',
  '✓ ◇ 已确认：答案回传续跑': '✓ $1 confirmed: answers returned for continuation',
  '✓ ◇ 核验通过：提交完整◇': '✓ $1 verification passed: commits complete$2',
  '作答保存失败：◇': 'Failed to save answers: $1',
  '保持挂起失败：◇': 'Failed to keep suspended: $1',
  '保持挂起说明：◇': 'Keep-suspended note: $1',
  '差异读取失败：◇': 'Failed to load diff: $1',
  '已保持挂起 ◇': 'Kept suspended $1',
  '已确认恢复：◇，核验通过，队列已恢复；条目验收仍走「确认完成」': 'Confirmed & resumed: $1, verification passed, queue resumed; item acceptance still goes through "Confirm done"',
  '补交 ◇ 组提交': 'supplemented $1 commit(s)',
  '最近核验（◇）：◇': 'Last verification ($1): $2',
  '未通过——◇': 'failed — $1',
