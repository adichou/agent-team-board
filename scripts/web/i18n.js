...
const EN = {
  ...
  '关键词在下方面板内前端过滤（编号 / 标题 / 执行器；排队批次含批次号），不发请求': 'Keywords filter the panel below on the client (ID / title / executor; queued batches include the batch ID); no request is sent',
  ...
  '子代理正在完善本文档；点击查看批量完善面板': 'A subagent is refining this document; click to view the batch refine panel',
  '存在': 'Present',
  '完善中': 'Refining',
  ...
  '到各项目的任务模块（「任务」页签）可启动新的批量开发 / 批量完善任务；新任务登记运行后会自动出现在这里。': 'Open the Tasks module of each project (the "Tasks" tab) to start new batch develop / refine tasks; newly registered runs appear here automatically.',
  ...
  '搜项目 / 批次号 / 条目编号…': 'Search projects / batch IDs / item IDs…',
  '摘要': 'Summary',
  '撤销': 'Undo',
  '操作': 'Actions',
  ...
  '暂无已计划候选：请先在看板接受条目，并在详情页「移入计划」（仅已计划且未被认领的条目可入批）。': 'No planned candidates: accept items on the board and "Add to plan" in the detail view (only planned and unclaimed items can join a batch).',
  '暂无待处理条目。': 'No pending items.',
  '暂无执行记录': 'No run records',
  '暂无调度提示词（存量批次可能缺失）。': 'No dispatch prompt (legacy batches may lack one).',
  ...
  '本批完善范围已处理完毕（条目保持已接受，后续流转由人工判断）。': 'This refine round is fully processed (items stay accepted; next steps are up to you).',
  ...
  '没有匹配的排队批次，清空搜索恢复。': 'No matching queued batches; clear the search to restore.',
  ...
  '输入框、文本域、下拉选择、contenteditable 及中文输入法组合输入期间不触发以上快捷键；带 Ctrl / ⌘ / Alt 的组合键与未列出的按键保留浏览器默认行为。快捷键只触发现有界面入口，不直接接受、计划、删除条目或启动批次。': 'These shortcuts do not fire while typing in inputs, textareas, selects, contenteditable, or during IME composition; combos with Ctrl / ⌘ / Alt and unlisted keys keep browser defaults. Shortcuts only trigger existing UI actions — they never accept, plan, delete items, or start batches.',
  '运行环境': 'Environment',
  '运行配置': 'Run config',
  '进入任务模块批量完善面板：对已接受未完善条目批量补全文档（与勾选无关）': 'Open the batch refine panel: complete documents of accepted unrefined items in bulk (independent of checkboxes)',
  '进入任务模块批量开发面板：以已计划队列（最旧优先）为范围，由面板内「启动」创建任务': 'Open the batch develop panel: scoped to the planned queue (oldest first); "Start" inside the panel creates the task',
  ...
  '驳回待接受（退回待接受）：勾选的已接受条目逐条退回待接受；完善中的单待本轮批量完善结束后再驳回': 'Reject to pending: selected accepted items return to pending one by one; refining items wait until this refine round ends',
  ...
  '以当前已计划候选创建新批次，FIFO 排到队尾，当前批次结束后自动接续': 'Create a new batch from current planned candidates, queued FIFO; starts automatically after the current batch',
  '任务分区': 'Task sections',
  ...
  '复制失败可重试；重试复制不会创建新批次': 'If copying fails you can retry; recopying does not create a new batch',
  ...
  '该批次当前无在途执行；删除仅移除批次账本，条目与运行记录不受影响，队列位次自动前移': 'No in-flight runs in this batch; deleting removes only the batch ledger — items and run records are unaffected and queue positions shift up',
  '详情分页': 'Detail paging',
  ...
  '本批完善范围已处理完毕（条目均保持已接受，后续流转由人工判断）。': 'This refine round is fully processed (items all stay accepted; next steps are up to you).',
  '本批完善范围已处理完毕（条目均保持已接受，后续流转由人工判断）': 'This refine round is fully processed (items all stay accepted; next steps are up to you)',
  ...
  '删除该排队批次（未在执行）：仅移除批次账本，候选条目回到可入批候选池，队列位次自动前移': 'Delete this queued batch (not running): removes only the batch ledger; candidates return to the pool and queue positions shift up',
  ...
  '完善中，待本轮批量完善结束后再驳回': 'Refining; reject after this refine round ends',
  ...
  '文档已由批量完善补全；点击查看批量完善面板': 'Documents completed by batch refine; click to view the batch refine panel',
  ...
  'worker 声明受阻待人工决策的条目在此承接：补决策 → 复工回已计划队列；清单不随批次结束消失': 'Items blocked on human decisions are handled here: answer decisions → resume back to the planned queue; the list persists after batches end',
  '查看进展记录': 'Progress log',
  '收起记录': 'Hide log',
  '补决策': 'Decide',
  '复工': 'Resume',
  ...
  '决策已齐备：条目回已计划队列，可被批量开发重新取单': 'All decisions answered: the item returns to the planned queue and can be picked up by batch development again',
...
const EN_DYNAMIC = {
  ...
  '已复工 ◇：回到已计划队列，可被批量开发重新取单': 'Resumed $1: back to the planned queue, ready for batch development to pick up again',
  ...
  '◇该批次不在执行中；仅移除批次账本，条目与运行记录不受影响，候选条目回到可入批候选池，队列位次自动前移。': '$1 This batch is not running; only its ledger is removed. Items and run records are unaffected, candidates return to the pool, and the queue shifts up automatically.',
  ...
  '✓ 任务已创建、提示词已复制，请在对应项目会话粘贴发送（批次 ◇，候选 ◇，受阻 ◇；状态：待启动）': '✓ Task created and prompt copied; paste it into a session of the corresponding project (batch $1, $2 candidates, $3 blocked; status: prepared)',
  ...
  '已切换：◇': 'Switched: $1',
  '已加入队列：批次 ◇ 排第 ◇ 位（重复创建返回已有排队批次）◇': 'Queued: batch $1 at position $2 (repeated creation returns the existing queued batch)$3',
  ...
  '排队批次（当前批次结束后同一调度会话自动接续）◇：': 'Queued batches (auto-continued by the same dispatch session after the current batch)$1:',
  ...
  '未完成 ◇': '$1 unfinished',
  '未新建完善任务：◇ 已有未结束的完善任务（幂等返回）◇': 'No new refine task: $1 already has an unfinished one (idempotent return)$2',
  '未新建批次：当前批次 ◇ 尚未结束◇': 'No new batch: the current batch $1 has not finished$2',
  ...
  '第 ◇ 位 · ◇ 项 · 创建 ◇◇': 'Position $1 · $2 items · created $3$4',
  '第 ◇ 次': 'Attempt #$1',
  '第 ◇ 行': 'Line $1',
  '第 ◇–◇ 行': 'Lines $1–$2',
  '等 ◇ 项': 'and $1 more',
  '简报类型字段缺失或未知（◇），已按批次号前缀◇。常驻看板服务的路由在启动时固化而静态前端实时读盘，服务进程可能旧于前端：请重启看板服务（atb serve / npm run app）后刷新；若重启后仍出现请按 BUG-20260911-007 反馈': 'The kind field is missing or unknown ($1); the batch-ID prefix was used$2. The resident board service fixes its routes at startup while the static frontend reads from disk live, so the service process may be older than the frontend: restart the board service (atb serve / npm run app) and reload; if it persists, report it as BUG-20260911-007',
  '终止批量完善任务 ◇？': 'Abort batch refine task $1?',
  '终止批量开发任务 ◇？': 'Abort batch develop task $1?',
  '编辑 ◇': 'Edit $1',
