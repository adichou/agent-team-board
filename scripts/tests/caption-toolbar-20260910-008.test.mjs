...
t('L4 按档显隐：待接受/已接受/已计划 全选与全不选可见，勾选首项 #selGroup 出现、清零隐藏；开发中/待测试/已完成 两按钮与 #selGroup 均隐藏；快捷入口已接受「开始完善」/已计划「开始开发」/已完成「开始 Commit」（BUG-20260911-005），其余档隐藏', () => {
  ...
  assert.equal(h.document.querySelector('#laneQuickEntry').textContent, '▶ AI 分析', '已接受档文案（REQ-20260914-005 去「开始」）');
  ...
  h.run('syncAcceptance()');
  assert.equal(h.document.querySelector('#laneQuickEntry').textContent, '▶ AI 开发', '已计划档文案（REQ-20260914-005 去「开始」）');
