...
t('T4 行为零变化：三选择档两图标按钮可见、非选择档隐藏；全选仅勾当前档∩搜索命中；全不选仅清当前档；零勾选禁全不选、无可操作项禁全选、批量进行中均禁用；勾选首项 #selGroup 出现、清零隐藏；快捷入口按档显隐文案且批量进行中不禁用', () => {
  ...
  assert.equal(h.document.querySelector('#laneQuickEntry').textContent, '▶ 开始完善', '已接受档快捷入口文案');
  ...
  assert.equal(h.document.querySelector('#laneQuickEntry').textContent, '▶ 开始开发', '已计划档快捷入口文案（镜像入口同口径）');
});
