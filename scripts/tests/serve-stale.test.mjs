...
// BUG-20260914-013 端口占用预检（pickFreePort）、就绪探测可校验 pid（命中冒牌 health 报端口
//       被占而非误导断言）、spawn 服务/stub 确定性收尾（try/finally + SIGTERM→有界等待→SIGKILL）。
...
import { pickFreePort, httpGetJson, waitHealth, stopChild, stopPid } from './lib/test-process.mjs';

const pluginRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..', '..');
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
const getJson = httpGetJson;
...
t('T1 /api/health 返回 pid 与 startedAt', async () => {
  ...
  const port = await pickFreePort({ min: 34100, max: 39099 });
  const server = startServer(path.join(pluginRoot, 'scripts', 'server.mjs'), tmp, port, path.join(tmp, 'reg.json'));
  try {
    // expectPid：命中端口上其他服务（残留/第三方）时不算就绪，超时报「端口被占」而非误导断言
    const h = await waitHealth(port, { expectPid: server.pid });
...
t('T3 服务不旧时 atb serve 照常复用、不误杀', async () => {
  ...
  fs.mkdirSync(proj, { recursive: true });
  const sk = copySkeleton(tmp);
  const port = await pickFreePort({ min: 35100, max: 40099 });
  const registry = path.join(tmp, 'reg.json');
  const cur = startServer(sk.server, proj, port, registry);
  try {
    const h0 = await waitHealth(port, { expectPid: cur.pid });
...
t('T5 无 startedAt 的旧服务被定位并重启（用户现场复刻）', async () => {
  ...
  fs.mkdirSync(proj, { recursive: true });
  const sk = copySkeleton(tmp);
  const port = await pickFreePort({ min: 35600, max: 40599 });
  const registry = path.join(tmp, 'reg.json');
  ...
  try {
    ...
    if (replacementPid && replacementPid !== stub.pid) await stopPid(replacementPid);
    try { fs.rmSync(tmp, { recursive: true, force: true }); } catch {}
  }
});
